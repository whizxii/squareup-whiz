...
export function MessageComposer({ onTypingChange }: MessageComposerProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    // ─── Agent Summoning: detect @agent mentions and invoke agents ───
    ...
    if (agentMentions.length > 0) {
      ...
      for (const mention of agentMentions) {
        const agent = agentStore.agents.find((a) => a.id === mention.id);
        ...
        agentStore.updateAgent(agentId, { status: "thinking", current_task: text });

        // Call the real backend API (fire-and-forget per mention)
        (async () => {
          try {
            const res = await fetch(`${apiUrl}/api/agents/${agentId}/invoke`, {
              method: "POST",
              headers: {
            ...
            // Add agent response to the channel
            useChatStore.getState().addMessage(channelId, {
              ...
              attachments: [],
              mentions: [],
              tool_calls: data.tools_called?.length > 0 ? data.tools_called : undefined,
            });
            ...
            console.error(`Failed to invoke agent ${agentId}:`, err);

            const errorMsgId = `agent-err-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
            useChatStore.getState().addMessage(channelId, {
              id: errorMsgId,
              channel_id: channelId,
              sender_id: agentId,
