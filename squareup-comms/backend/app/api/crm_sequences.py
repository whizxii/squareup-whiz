...
class EnrollmentResponse(BaseModel):
    ...
    @classmethod
    def from_model(
        cls, enr: Any, *, contact_name: Optional[str] = None
    ) -> "EnrollmentResponse":
        return cls(
            id=enr.id,
...
async def get_sequence_enrollments(
    ...
    contact_ids = [e.contact_id for e in page.items]
    name_map: dict[str, str] = {}
    if contact_ids:
        result = await session.execute(
            select(CRMContact.id, CRMContact.name).where(
    ...
    return success_response({
        "items": [
            EnrollmentResponse.from_model(e, contact_name=name_map.get(e.contact_id))
...
async def get_enrollments_for_contact(
    ...
    if enrollments:
        result = await session.execute(
            select(CRMContact.name).where(CRMContact.id == contact_id)
        )
        contact_name = result.scalar_one_or_none()
    return success_response([
        EnrollmentResponse.from_model(e, contact_name=contact_name).model_dump(mode="json")
